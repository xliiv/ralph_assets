...
class XlsUploadView(SessionWizardView, AssetsBase):
    ...
    # new code
    # TODO:
    # TODO:: add check if model has test2field_value else raise exception
    # TODO:: class RequiredFieldError(Exception):

    def dry_run_dunno(self):
        pass

    def register_result(self, row, exc):
        '''
        - register result for user per item (from imported items)
        '''
        # TODO:: add record to db??
        msg = getattr(exc, 'message', None) or repr(exc)
        msg = 'Unable to load row: {row!r} because of: {exc_msg}'.format(
            row=row, exc_msg=msg,
        )
        errors = self.ctx_data.setdefault('errors', [])
        errors.append(msg)
        self.ctx_data['errors'] = errors

    @transaction.commit_on_success
    def done(self, form_list):
        #TODO:: make if flat or something, now digging out from inherited design
        #TODO:: handle mapping from choices
        self.ctx_data = self.get_context_data()
        rows_to_import = self.get_cleaned_data_for_step('upload')['file']
        rows_to_import = rows_to_import[2]['csv']
        model = Asset

        csv_col2model_field = self.storage.data['mappings']
        for row in rows_to_import:
            object_data = {}
            if 'id' in row:
                try:
                    instance = model.objects.get(pk=row.pop('id'))
                except model.DoesNotExist as exc:
                    self.register_result(row, exc)
                    continue
            else:
                instance = model()
            try:
                for csv_fieldname, text_value in row.iteritems():
                    csv_fieldname = slugify(csv_fieldname)
                    if csv_fieldname not in csv_col2model_field:
                        continue
                    model_fieldname = csv_col2model_field[csv_fieldname]
                    try:
                        field_value = instance.text2field_value(
                            model_fieldname, text_value,
                        )
                    except RequiredFieldError as exc:
                        self.register_result(row, exc)
                        break
                    object_data[model_fieldname] = field_value
                else:
                    print('object_data', object_data)
                    instance.update_model(object_data)
            # TODO:: uncomment when ready except Exception as exc:
            except OffException as exc:
                self.register_result(row, exc)
        return render(
            self.request,
            'assets/xls_upload_wizard_done.html',
